...
pub fn init(level_filter: LevelFilter, enable_colors: bool) -> Result<(), SetLoggerError> {
    log::set_boxed_logger(Box::new(NGMPLogger {
        level_filter,
        enable_colors,
        max_record_level: AtomicUsize::new(0),
    }))
...
impl log::Log for NGMPLogger {
    ...
    fn log(&self, record: &Record) {
        if self.enabled(record.metadata()) {
            ...
            let filler = if level == Level::Info || level == Level::Warn {
                " "
            } else {
                ""
            };
            let color = if self.enable_colors {
                match level {
            ...
            let exp_len = self
                .max_record_level
                .fetch_max(target.len(), Ordering::AcqRel)
                .max(target.len());
            ...
            println!(
                "{color}[{level}]{color_reset} {filler} {target} {target_filler} {}",
                record.args()
            )
        }
    }

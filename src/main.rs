...
fn client_accept_thread(config: Config, tx: mpsc::Sender<Client>) {
    ...
    let handle = rt.spawn(async move { client_accept_async(config, tx).await; });
...
async fn accept_client(mut tcp_conn: TcpConnection<Packet>, addr: std::net::SocketAddr, config: &Config) -> Option<Client> {
...
async fn client_accept_async(config: Config, tx: mpsc::Sender<Client>) {
    ...
    let tcp_listener = tokio::net::TcpListener::bind(&tcp_addr).await.expect("Failed to bind TCP socket!");
...
async fn main() {
    ...
    let udp_listener = UdpListener::<Packet>::bind(&udp_bind_addr).await.map_err(|e| error!("{}", e)).unwrap();
